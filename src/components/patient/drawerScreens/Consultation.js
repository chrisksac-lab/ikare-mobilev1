      ...
      if (tel.trim().length == 0) {
        ...
        try {
         setPaymentLoading(true);
          ...
          const response = await axiosInstance.post(url, {tel_fee:tel, ...user, details:"Consultation fee", amount: 10, doctor});
          if (response.status == 200)
          {
            Toast.show({
             ...
             .then(() => {
                console.log(appointment?.consultation?.room);
                setCall(_call)
             })
             .catch(error => {
              Toast.show({
                type:"error",
                text1: "An error occured !, internet connection"
              })
             console.log(error)})
          }
          else 
      ...
      <ScrollView style={tw`px-8 py-4`}>       
                ...
                <Text style={tw`p-4 text-center flex items-center justify-center text-blue-900 text-[20px] font-bold w-full`}>Make a payment</Text>
                      ...
                      />
                        ...
                        { !paymentLoading ? <Pressable onPress={() => makePayments()} style={tw`rounded-[10px] bg-teal-900 p-3`}>
                          <Text style={tw`text-white font-bold text-center`}>Initiate payment</Text>
                      </Pressable> : <ActivityIndicator size={48} />}
